...
class Ion_auth
{
	 ...
	 * @author Mathew
	 **/
	public function forgotten_password($identity, $settings)    //changed $email to $identity
	{
		if ( $this->ion_auth_model->forgotten_password($identity) )   //changed
		{
			...
			if ($user)
			{
				...
				{
					$from_email = (!empty($settings->email)) ? $settings->email : $this->config->item('admin_email', 'ion_auth') ; 
					$site_name = (!empty($settings->site_title)) ? $settings->site_title : $this->config->item('site_title', 'ion_auth') ;

					$subject   = 'Password Reset Information';
					$message   = '<h2>Hello '.$user->username.', </h2>';
					$message  .= '<p>We have received a request to reset your password,</p>';
					$message  .= '<p> If you want to reset your password, click on the link below (or copy and paste the URL into your browser)</p>';
					$message  .= '<p>'.site_url('auth/reset_password/'.$user->forgotten_password_code).'</p>';
					$message  .= '<p>Best Regards.</p>';
					$message  .= '<p>Webmaster</p>';
					$message  .= '<hr/>';
					$message  .= '<p style="font-size:8px; color:#969AB3">Please do not reply to this message; it was sent from an unmonitored email address. This message is a service email related to your use of the system . For general inquiries or to request support with your account, please contact us for Support.</p>';
					
					$this->email->clear();
					$this->email->from($from_email, $site_name);
					$this->email->to($user->email);
					$this->email->subject($site_name . ' - ' . $this->lang->line('email_forgotten_password_subject'));
	 ...
	 * @author Ben Edmunds
	 **/
	public function is_admin($id=false,$groupasb='admin_group')
	{
		$this->ion_auth_model->trigger_events('is_admin');

		$admin_group = $this->config->item($groupasb, 'ion_auth');
		//print_r($this->in_group($admin_group, $id));
		//print_r($admin_group);
		//die();
